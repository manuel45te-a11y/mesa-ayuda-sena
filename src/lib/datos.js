...
export async function asignarTecnico(ticketId, tecnicoId, comentario) {
  if (demo.esDemo()) {
    return await demo.asignarTecnico(ticketId, tecnicoId, comentario);
  }
  ...
  if (errTicket) return { ticket: null, error: errTicket.message };
  ...
  await supabase.from('ticket_eventos').insert({
    ...
    comentario: comentario || 'Técnico asignado por el Administrador',
  });
...
export async function subirEvidencia(ticketId, { imagen, descripcion, nombre }, usuarioId) {
  if (demo.esDemo()) {
    return await demo.subirEvidencia(ticketId, { imagen, descripcion, nombre });
  }
  ...
  if (error) return { evidencia: null, error: error.message };

  await supabase.from('ticket_eventos').insert({
    ticket_id: ticketId,
    tipo: 'evidencia',
    comentario: `Evidencia técnica adjuntada: ${nombre || 'Archivo'}${descripcion ? ` — ${descripcion}` : ''}`,
  });

  return { evidencia: data, error: null };
}
...
export async function obtenerEvidencias(ticketId) {
  ...
  return {
    evidencias: (data ?? []).map((e) => ({
      ...e,
      subido_por_nombre: e.perfiles?.nombre,
    })),
    error: error?.message ?? null,
  };
}

export async function cerrarTicket(ticketId, { solucion, tecnicoId }, usuarioId) {
  if (demo.esDemo()) {
    return await demo.cerrarTicket(ticketId, { solucion, tecnicoId });
  }
  ...
  if (errCount) return { error: errCount.message };
  if (!count || count === 0) {
    return {
      error: 'Es obligatorio adjuntar al menos una evidencia técnica antes de cerrar la solicitud.',
    };
  }
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      tecnico_id: tecnicoId || usuarioId,
