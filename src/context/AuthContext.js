...
export function AuthProvider({ children }) {
  ...
  async function cargarPerfil(userId) {
    ...
    const p = data ?? null;
    if (p && (p.correo?.toLowerCase() === 'larioscarvajal2007@gmail.com')) {
      p.rol = 'admin';
    }
    setPerfil(p);
  }
  ...
  const esAdminPorCorreo =
    (perfil?.correo?.toLowerCase() === 'larioscarvajal2007@gmail.com') ||
    (sesion?.user?.email?.toLowerCase() === 'larioscarvajal2007@gmail.com');

  const perfilActivo = demo
    ? obtenerPerfilDemo()
    : perfil
      ? (esAdminPorCorreo ? { ...perfil, rol: 'admin' } : perfil)
      : null;

  const rol = perfilActivo?.rol ?? (esAdminPorCorreo ? 'admin' : 'aprendiz');
  ...
  const valor = useMemo(
    () => ({
      ...
      rol,
      esSoporte: rol === 'tecnico' || rol === 'admin' || esAdminPorCorreo,
      esAdmin: rol === 'admin' || esAdminPorCorreo,
      esTecnico: rol === 'tecnico' && !esAdminPorCorreo,
      esUsuario: (rol === 'aprendiz' || rol === 'usuario') && !esAdminPorCorreo,

      // Permite alternar de rol inmediatamente en modo demo para probar los 3 perfiles
      ...
      // la cuenta esperando.
      async registrar({ correo, clave, nombre, rol: rolSeleccionado, ficha, programa, telefono }) {
    ...
    [sesion, perfilActivo, cargando, demo, caducada, demoTick, rol]
  );
